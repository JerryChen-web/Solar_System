...
import { pickBodyId } from "./rendering/selection";
...
export class SolarSystemApp {
  ...
  private animate = (): void => {
    ...
    if (!this.paused && this.mode === "kepler") {
    ...
    this.controlPanel.setSimulationDate(formatSimulationDate(this.secondsSinceEpoch));
  ...
  private selectBody(bodyId: string): void {
    ...
    for (const [id, node] of this.bodyNodes) {
      node.mesh.scale.setScalar(id === bodyId ? 1.18 : 1);
    }
  ...
  private updateSelectedBodyInfo(bodyId: string): void {
    ...
    this.bodyInfoPanel.update(body, this.relativeDistanceMeters.get(bodyId) ?? 0);
  }
}
